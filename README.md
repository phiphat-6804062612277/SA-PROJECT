...
"แสดงในร้าน" หรือ "ขายนอกร้าน (แสดงเฉพาะหน้าตลาด)" · เมนูแยกตามงาน (หน้าร้าน / ออเดอร์ / สินค้า / ข้อความ / แดชบอร์ด) ·
...
ที่ไม่เหมาะสม (หน้า `/admin/users`) · ภาพรวมระบบ (`/admin`) · **Dispute Resolution Panel** ไกล่เกลี่ยข้อพิพาท (หน้า `/admin/disputes`)
...
**ระบบแจ้งเตือนงานค้าง (Notification Badge)** — จุดแดง `!` แสดงที่ **เมนูที่เกี่ยวข้องในแถบเมนูล่าง (และเมนู ☰)** ไม่แสดงที่รูปโปรไฟล์อีกแล้ว:
ผู้ซื้อ → เมนู "คำสั่งซื้อ" เมื่อมีออเดอร์ `SHIPPED` รอกด "ยืนยันรับสินค้า" · ผู้ขาย → เมนู "ออเดอร์" เมื่อมีออเดอร์ใหม่รอจัดส่ง/กรอกเลขพัสดุ (รวมข้อพิพาทที่ต้องชี้แจง) ·
Admin → เมนู "ข้อพิพาท" เมื่อมี Dispute `PENDING` รอตัดสิน · **ทุกบทบาท** → เมนู "ข้อความ" (Admin: "Support") เมื่อมีข้อความแชตที่ยังไม่ได้อ่าน ·
อัปเดตทุก 45 วินาที เมื่อเปลี่ยนหน้า และเมื่อกลับมาที่แท็บ (`GET /api/notifications` คีย์: `buyer_confirm` · `seller_ship` · `seller_dispute` · `admin_disputes` · `chat_unread`)

**เมนูนำทางแยกตามบทบาท (Bottom Navigation มีป้ายชื่อใต้ไอคอน)** — ไม่กองเมนูไว้ที่หน้าโปรไฟล์ (หน้าโปรไฟล์เหลือข้อมูลส่วนตัว รูปโปรไฟล์ รีวิวที่ได้รับ และออกจากระบบ):

| บทบาท | เมนูทั้ง 5 |
|---|---|
| Buyer | หน้าแรก (`/`, ค้นหา/ตะกร้าอยู่ที่หัวหน้า) · คำสั่งซื้อ (`/history`) `!` · ข้อความ (`/chat`) `!` · Wallet (`/wallet`) · โปรไฟล์ (`/profile`) |
| Seller | หน้าร้าน (`/store/[ตัวเอง]`) · ออเดอร์ (`/seller/orders`) `!` · สินค้า (`/seller/products`) · ข้อความ (`/chat`) `!` · แดชบอร์ด (`/seller` — สรุป/Wallet-ถอนเงิน/ตั้งค่าร้าน) |
| Admin | ภาพรวม (`/admin`) · ข้อพิพาท (`/admin/disputes`) `!` · ผู้ใช้/ร้าน (`/admin/users`) · Support (`/chat`) `!` · โปรไฟล์ (`/profile`) |
| ยังไม่ล็อกอิน | หน้าแรก · สินค้า · ตะกร้า · เข้าสู่ระบบ |

โครงเมนูอยู่ที่ `frontend/src/lib/nav.js` (ใช้ร่วมกับเมนู ☰) — แก้ที่เดียวเปลี่ยนทั้งสองที่
...
- **Buyer/Seller ↔ Admin** — "ติดต่อ Admin" จากเมนูโปรไฟล์/หน้า `/chat` (`type = SUPPORT`, `topic = HELP`) ทีม Admin เห็นทุกห้องซัพพอร์ตในกล่องข้อความ `/chat` (กรอง ยังไม่อ่าน / ยื่นอุทธรณ์ / เปิดอยู่ / ปิดแล้ว, ปิด-เปิดเรื่อง, ปลดแบนบัญชี/ร้านจากในห้อง) และเริ่มคุยกับผู้ใช้จากหน้า `/admin/users` ได้
...
- **โครงสร้างข้อมูล** — `Message`: `senderId`, `receiverId` (`null` = ทีม Admin), `conversationId`, `messageType` (`TEXT | IMAGE | FILE`), `text`, `fileUrl`, `fileName/fileMime/fileSize`, `isSupportChat` ·
  `Conversation`: `ownerId`, `peerId`, `type`, `topic`, `status` (`OPEN|CLOSED`) + `closedAt/closedBySide/closedByRole`, ตัวนับข้อความใหม่ของแต่ละฝ่าย · ข้อความในข้อพิพาท (`Dispute.messages`) มีฟิลด์ไฟล์แนบชุดเดียวกัน
- **สถานะห้อง / ปิดแชต (Close Chat)** — ทุกห้องมีป้ายสถานะ `OPEN` / `CLOSED` ที่หัวห้อง สมาชิกในห้อง (ผู้ซื้อ / ผู้ขาย / Admin ในห้องซัพพอร์ต) กด **"ปิดการสนทนา (Close Chat)"** ได้
  ห้องที่ปิดจะแสดง Banner "การสนทนานี้ถูกปิดแล้ว" (บอกว่าใครปิด/เมื่อไร) ล็อกช่องพิมพ์และการแนบไฟล์ (อ่านได้อย่างเดียว) และกด "เปิดการสนทนาอีกครั้ง" ได้ (`PUT /api/chat/conversations/:id/status`)
  — ผู้ที่ถูกแบนเปิดห้องอุทธรณ์ที่ Admin ปิดไว้ซ้ำได้เอง (ไม่งั้นจะอุทธรณ์ต่อไม่ได้) · **แชตข้อพิพาท** ปิดอัตโนมัติเมื่อ Admin ตัดสิน (`chatStatus = CLOSED`) และไม่มีปุ่มเปิดใหม่
- **อื่นๆ** — ดึงข้อความใหม่ทุก 4 วินาที (`?after=<id>` เฉพาะที่ใหม่กว่า, แท็บที่ซ่อนอยู่จะพัก) และโหลดข้อความเก่าด้วย `?before=<id>` · จำกัด 8 ข้อความ/10 วินาที/บัญชี · ข้อความยาว ≤ 1,000 ตัวอักษร ·
  ข้อความที่ยังไม่อ่านขึ้น Badge ที่เมนู "ข้อความ" (`GET /api/notifications` มีรายการ `chat_unread`)
...
| ตะกร้า · ชำระเงิน · คำสั่งซื้อของฉัน · โปรไฟล์ · Wallet | `/cart` · `/checkout` · `/history` · `/profile` · `/wallet` |
| Seller: แดชบอร์ด · ออเดอร์ (`?filter=todo`) · จัดการสินค้า · ตั้งค่าร้าน (โลโก้/แบนเนอร์) · เพิ่ม/แก้ไขสินค้า | `/seller` · `/seller/orders` · `/seller/products` · `/seller/store` · `/seller/products/new` · `/seller/products/[id]` |
...
| Admin: ภาพรวม · ผู้ใช้ & ร้านค้า · จัดการข้อพิพาท · พิจารณาข้อพิพาท | `/admin` · `/admin/users` · `/admin/disputes` · `/admin/disputes/[id]` |
...
6. ล็อกอินเป็น Admin → เมนู "ผู้ใช้/ร้าน" (`/admin/users`) เพื่อลองแบนผู้ใช้/แบนร้าน/ลบสินค้า
...
| GET | `/api/notifications` | ล็อกอิน | งานค้างตามบทบาท `{ total, items[] }` (ใช้แสดง Badge ที่เมนูแต่ละเมนู) |
...
| PUT | `/api/chat/conversations/:id/status` | สมาชิกห้อง / admin | ปิด/เปิดการสนทนา `{ status: OPEN\|CLOSED }` (ปิด = ส่งข้อความ/ไฟล์ไม่ได้) |
...
- การแจ้งเตือนมีเฉพาะ Badge ที่เมนูในหน้าเว็บ (ไม่มีอีเมล/Push) และใช้การดึงข้อมูลเป็นระยะ ไม่ใช่ Real-time

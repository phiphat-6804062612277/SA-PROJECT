...
**ผู้ขาย** (เลือกบทบาทตอนสมัคร) — **ตั้งชื่อ/คำอธิบายร้านค้า** · ลง/แก้ไข/ลบสินค้า (สินค้าทุกชิ้นผูกกับร้านของผู้ขายที่ลงเสมอ
แสดงทั้งในหน้าร้านและตลาดรวม — ไม่มีตัวเลือก "ขายนอกร้าน") · เมนูแยกตามงาน (หน้าร้าน / ออเดอร์ / สินค้า / ข้อความ / แดชบอร์ด) ·
...
- **Buyer/Seller ↔ Admin** — "ติดต่อ Admin" จากเมนูโปรไฟล์/หน้า `/chat` (`type = SUPPORT`, `topic = HELP`) ทีม Admin เห็นทุกห้องซัพพอร์ตในกล่องข้อความ `/chat` (กรอง ยังไม่อ่าน / ยื่นอุทธรณ์, ปลดแบนบัญชี/ร้านจากในห้อง) และเริ่มคุยกับผู้ใช้จากหน้า `/admin/users` ได้
...
- **โครงสร้างข้อมูล** — `Message`: `senderId`, `receiverId` (`null` = ทีม Admin), `conversationId`, `messageType` (`TEXT | IMAGE | FILE`), `text`, `fileUrl`, `fileName/fileMime/fileSize`, `isSupportChat` ·
  `Conversation`: `ownerId`, `peerId`, `type`, `topic`, ตัวนับข้อความใหม่ของแต่ละฝ่าย · ข้อความในข้อพิพาท (`Dispute.messages`) มีฟิลด์ไฟล์แนบชุดเดียวกัน
- **ไม่มีระบบปิดแชต** — ห้องสนทนาทุกห้อง (ซื้อขาย / ติดต่อ Admin / อุทธรณ์) ไม่มีสถานะ `OPEN/CLOSED` และไม่มีปุ่มปิดการสนทนา ส่งข้อความและแนบไฟล์ได้ตลอดเวลา
  (ข้อจำกัดเดียวคือแชตซื้อขายส่งหาบัญชีที่ถูกแบนไม่ได้) · แชตข้อพิพาทยังส่งได้จนกว่า Admin จะตัดสิน ตัดสินแล้วข้อพิพาทเป็นอันจบ ไม่รับข้อความเพิ่ม (กติกาของข้อพิพาท ไม่ใช่สถานะห้อง) ·
  ตอนเริ่มเซิร์ฟเวอร์ `utils/migrations.js` จะลบฟิลด์สถานะ/ข้อมูลการปิดห้องที่ค้างจากระบบเดิมออกจากฐานข้อมูล ห้องที่เคยถูกปิดจึงกลับมาใช้งานได้
...
- **Single Active Session (1 บัญชี = 1 เครื่อง/เบราว์เซอร์ ณ เวลาเดียวกัน)** — ทุกครั้งที่ล็อกอิน/สมัคร ระบบสุ่มรหัสเซสชันใหม่เก็บที่ `User.sessionId` (ซ่อนจาก query ปกติ `select:false`) และฝังใน JWT เป็น `sid`
  Middleware `auth` เทียบ `sid` กับค่าในฐานข้อมูล **ทุกคำขอ** — เครื่องที่ล็อกอินไว้ก่อนหน้าจะได้ `401 { code: 'SESSION_REPLACED' }` ทันทีที่เรียก API ถัดไป หน้าเว็บ (`lib/api.js`) ล้างเซสชันแล้วพาไปหน้า Login พร้อมข้อความแจ้ง
  (ตัวเช็กคือคำขอแจ้งเตือนที่ยิงทุกครั้งที่เปลี่ยนหน้า / ทุก 45 วินาที / เมื่อกลับมาที่แท็บ และแชตที่ดึงทุก 4 วินาที) · ออกจากระบบเรียก `POST /api/auth/logout` ล้างเซสชันในฐานข้อมูล ·
  เปลี่ยนรหัสผ่านด้วย OTP = ทุกเซสชันเดิมเป็นโมฆะ · โทเคนรุ่นเก่าที่ไม่มี `sid` ใช้ไม่ได้ (ต้องล็อกอินใหม่ 1 ครั้งหลังอัปเดต) · ผู้ที่ถูกแบนล็อกอินได้แค่ appeal token จึงไม่ไปแทนที่เซสชันเดิมของใคร
...
| ผู้ขาย | `seller@solify.test` | ร้าน "โซลาร์ไทย สโตร์" มีสินค้าตัวอย่าง 5 รายการ |
...
Single Active Session (ล็อกอินซ้ำเด้งเครื่องเดิม / logout / รีเซ็ตรหัสผ่าน / โทเคนปลอม), ร้านค้า (สินค้าทุกชิ้นผูกกับร้านผู้สร้าง), แชตไม่มีสถานะปิดห้อง, รีวิว/เรตติ้ง, Admin แบน/ปลดแบน/ลบสินค้า, OTP ลืมรหัสผ่าน,
...
|---|---|---|---|
| POST | `/api/auth/register` `/login` | – | สมัคร (role = buyer/seller/admin*) / เข้าสู่ระบบ — ทั้งคู่ออกเซสชันใหม่ (Single Active Session) |
| POST | `/api/auth/logout` | ล็อกอิน | ออกจากระบบ: ล้างเซสชันในฐานข้อมูล โทเคนที่ถืออยู่ใช้ไม่ได้อีก |
...
| POST/PUT/DELETE | `/api/products` `/:id` | seller (เจ้าของ) | ลง/แก้/ลบสินค้า (ผูกกับร้านของผู้ขายจาก token เสมอ — `sellerId` ใน body ไม่มีผล) |
...
| GET | `/api/stores/:sellerId` | – | หน้าร้าน (โลโก้/แบนเนอร์/ขายแล้ว) + สินค้าทั้งหมดของร้าน + เรตติ้ง |
...
| GET | `/api/chat/conversations` | ล็อกอิน (รวมผู้ถูกแบน) | รายการห้องของฉัน (Admin: ห้องซัพพอร์ตทั้งหมด `?unread=1&topic=` + `counts`) |

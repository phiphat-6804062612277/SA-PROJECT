...
test('direct chat: seller opens from an order, pagination, blocked when the other party is banned, spam limit', async () => {
  ...
  assert.equal((await read(seller, id)).body.conversation.context.href, '/seller?tab=orders');
...
test('banned user appeal: login gives a limited appeal token that only opens the Admin support chat', async () => {
  ...
  // Admin ปิดเรื่อง → ผู้ใช้ส่งข้อความใหม่ = เปิดเรื่องอีกครั้ง
  assert.equal((await api().put(`/api/chat/conversations/${sid}/status`).set(as(a)).send({ status: 'CLOSED' })).status, 200);
  assert.equal((await list(a, '?status=OPEN')).conversations.length, 0);
  assert.equal((await api().put(`/api/chat/conversations/${sid}/status`).set(as(appeal)).send({ status: 'CLOSED' })).status, 403);
  assert.equal((await api().put(`/api/chat/conversations/${sid}/status`).set(as(a)).send({ status: 'nope' })).status, 400);
  await send(appeal, sid, { text: 'ขอบคุณครับ' });
